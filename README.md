...
데스크탑·랩탑 브라우저에서 키보드로 즐기는 초고속 테트리스 웹 게임입니다. 조각이 나오자마자 바닥에 꽂히는 20G 방식이라, 락 딜레이(0.25초, 이동·회전 시 최대 15회 연장) 안에 밀고 돌려서 자리를 잡아야 합니다. 빌드 도구 없이 순수 HTML / CSS / JavaScript로 만들어져 있어 로컬 및 Vercel 등 정적 호스팅에 그대로 올릴 수 있으며, Cloudflare Worker + D1 Database를 통한 **실시간 온라인 랭킹 시스템**이 기본 연동되어 있습니다.

---

## 🎮 주요 기능

- **표준 가이드라인 & SRS All-Mini 룰**: 7-bag 랜덤, SRS 회전(월 킥), All-Mini 룰(모든 미노 스핀 인정 & B2B 유지), 고스트 피스, 홀드, NEXT 프리뷰
